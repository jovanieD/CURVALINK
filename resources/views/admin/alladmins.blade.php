                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                    <div class="row mb-2">
                        <div class="col-md-6">
                            ...
                            <a href="/admin_add_admin"><button id="addScheduleButton" class="btn btn-info m-2">+
                                    Add Admin</button></a>
                        </div>

                        <div class="col-md-6">
                            <form method="GET" action="{{ route('searchAdmin') }}" class="form-inline">
                                @csrf
                    ...
                    <div class="card">
                            ...
                            <table class="table">
                                ...
                                <tbody class="table-border-bottom-0">
                                    ...
                                    @foreach ($admin as $adminData)
                                        @include('modals.admindeleteadmin')
                                        <tr>
                                            <td>{{ $adminData['firstname'] }} {{ $adminData['lastname'] }}</td>
                                            <td>{{ $adminData['rank'] }}</td>
                                            <td>{{ $adminData['gender'] }}</td>
                                            <td>{{ $adminData['email'] }}</td>
                                            <td>{{ $adminData['subject_handle'] }}</td>
                                            <td>{{ $adminData['grade_level'] }}</td>
                        ...
                        <nav class="d-flex justify-content-center m-2">
                            <ul class="pagination">
                                {{ $admin->onEachSide(1)->setPath('/all_admins')->appends(request()->query())->links('pagination::bootstrap-4') }}
                            </ul>
                        </nav>
